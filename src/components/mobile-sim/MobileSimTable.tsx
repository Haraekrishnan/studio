...
export default function MobileSimTable({ onEdit }: MobileSimTableProps) {
    const { user, roles, mobileSims, projects, users, deleteMobileSim } = useAppContext();
    ...
    if (mobileSims.length === 0) {
        return <p className="text-muted-foreground text-center py-8">No Mobile/SIM records found.</p>;
    }
                    ...
                    return (
                        <TableRow key={item.id}>
                            ...
                            <TableCell>
                                ...
                                <Avatar className="h-7 w-7"><AvatarImage src={allottedUser?.avatar} /><AvatarFallback>{allottedUser?.name.charAt(0)}</AvatarFallback></Avatar>
                                {allottedUser?.name || 'N/A'}
                                </div>
                            </TableCell>
                            <TableCell><Badge variant={item.status === 'Active' ? 'secondary' : 'default'}>{item.status}</Badge></TableCell>
                            {canManage && (
                                <TableCell className="text-right">
                                    <AlertDialog>
                                        ...
                                        <AlertDialogContent>
                                            <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>This action will permanently delete the record.</AlertDialogDescription></AlertDialogHeader>
