...
export default function InventoryFilters({ onApplyFilters }: InventoryFiltersProps) {
    ...
    return (
        <div className="flex flex-wrap gap-4 items-center">
            ...
            <Select value={status} onValueChange={setStatus}><SelectTrigger className="w-full md:w-[180px]"><SelectValue placeholder="Filter by status..." /></SelectTrigger><SelectContent><SelectItem value="all">All Statuses</SelectItem>{statusOptions.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select>
